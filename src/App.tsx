...
function chooseBestYT(vs: any[]): string | null {
  ...
  const yt = list.filter((v) => v && v.site === 'YouTube' && (v.type === 'Trailer' || v.type === 'Teaser'));
  if (!yt.length) return null;
  const score = (v: any) => {
    const name = String(v?.name || '').toLowerCase();
    let s = 0;
    if (v.type === 'Trailer') s += 5; // Trailer > Teaser
    if (v.official) s += 4; // TMDB official flag
    if (name.includes('official trailer')) s += 4;
    if (name.includes('trailer')) s += 1;
    if (v.iso_639_1 === 'en') s += 2; // prefer English
    const t = Date.parse(v.published_at || v.publishedAt || '');
    if (!Number.isNaN(t)) {
      const years = (Date.now() - t) / (1000 * 60 * 60 * 24 * 365);
      if (years < 3) s += 1; // mild bump for recent
    }
    return s;
  };
...
// ===== Mini runtime tests (console only) =====
function runTests() {
  const t = (name: string, fn: () => any) => {
    try { console.log('TEST \u2713', name, fn()); }
    catch (e) { console.error('TEST \u2717', name, e); }
  };
  ...
  t('chooseBestYT prefers official trailer', () => {
    const key = chooseBestYT([
      { site: 'YouTube', type: 'Teaser', key: 'A', name: 'Teaser', official: false, iso_639_1: 'en' },
      { site: 'YouTube', type: 'Trailer', key: 'B', name: 'Official Trailer', official: true, iso_639_1: 'en' },
      { site: 'YouTube', type: 'Trailer', key: 'C', name: 'Trailer', official: false, iso_639_1: 'en' },
    ]);
    if (key !== 'B') throw new Error('did not pick official');
    return key;
  });
  t('chooseBestYT returns null when none', () => chooseBestYT([]) === null);
}
...
const WinnerCard = ({ movie, tmdbUrl }: { movie: any; tmdbUrl: string }) => {
  ...
  return (
    <div className="p-6 rounded-2xl border border-neutral-800 bg-neutral-900/50 max-w-md mx-auto h-[680px] flex flex-col overflow-hidden text-center">
      {poster ? (
        <img src={poster} alt={movie.title} className="w-2/3 mx-auto aspect-[2/3] rounded-lg mb-4 overflow-hidden border border-neutral-800 bg-neutral-800/50 object-cover" />
      ) : (
...
const safeMovie = (m?: MovieLite | null) => ({
  id: m?.id ?? -1,
...
const Card = ({ movie, genres, onSwipe }: { movie?: MovieLite | null; genres?: { id: number; name: string }[]; onSwipe?: (dir: 'left' | 'right') => void; }) => {
  const [imgOk, setImgOk] = useState(true);
  const [dx, setDx] = useState(0);
  const [drag, setDrag] = useState(false);
  const [start, setStart] = useState<number | null>(null);
  const m = useMemo(() => safeMovie(movie || undefined), [movie]);
  ...
  const end = () => {
    ...
    if (d) onSwipe?.(d);
    setDx(0); setDrag(false); setStart(null);
  };
        ...
        {!!m.genre_ids.length && (
          <div className="flex flex-wrap gap-2 text-xs text-neutral-400 mt-auto pt-2">
            {m.genre_ids.map((id: number) => (
              <span key={id} className="px-2 py-1 rounded-lg bg-neutral-800">{g.find((x) => x.id === id)?.name || id}</span>
            ))}
          </div>
        )}
      </div>
...
const TrailerReview = ({ user, token, ids: onIds, onDone }: { user: User; token: string; ids: number[] | undefined; onDone: (ids: number[]) => void; }) => {
  ...
  const [tr, setTr] = useState<string | 'none' | null>(null);
  ...
  useEffect(() => {
    let live = true;
    setMeta(null); setTr(null); setPlaying(false);
    if (!id) return;
    (async () => {
      try {
  ...
  const next = (ok: boolean) => {
    ...
    if (i + 1 >= ids.length) onDone(picks); else { setYes(picks); setI(i + 1); }
  };
      ...
      >
        <div className="w-2/3 mx-auto aspect-[2/3] rounded-lg mb-4 overflow-hidden border border-neutral-800 bg-neutral-800/50 relative">
          ...
          {playing && tr && tr !== 'none' && (
            <div className="fixed inset-0 z-50 bg-black/90">
              <button aria-label="Close trailer" onClick={() => setPlaying(false)} className="absolute top-3 right-3 z-[70] px-3 py-1.5 rounded-lg bg-neutral-900/80 border border-neutral-700 text-sm"><I.X /> Close</button>
              {/* swipe capture rails (allow swipe while video plays) */}
              <div className="absolute inset-y-0 left-0 w-[22%] z-[70] cursor-ew-resize" onTouchStart={(e) => { setStart(e.touches[0].clientX); setDrag(true); }} onTouchMove={(e) => move(e.touches[0].clientX)} onTouchEnd={end} onMouseDown={(e) => { if (e.button!==0) return; setStart(e.clientX); setDrag(true); }} onMouseMove={(e) => { if (start!=null) move(e.clientX); }} onMouseUp={end} />
              <div className="absolute inset-y-0 right-0 w-[22%] z-[70] cursor-ew-resize" onTouchStart={(e) => { setStart(e.touches[0].clientX); setDrag(true); }} onTouchMove={(e) => move(e.touches[0].clientX)} onTouchEnd={end} onMouseDown={(e) => { if (e.button!==0) return; setStart(e.clientX); setDrag(true); }} onMouseMove={(e) => { if (start!=null) move(e.clientX); }} onMouseUp={end} />
              ...
              <div className="absolute inset-x-0 top-0 flex justify-center" style={{ paddingTop: 'calc(env(safe-area-inset-top, 0px) + 16px)' }}>
                <div className="w-screen">
                  <div className="relative w-full" style={{ paddingTop: '56.25%' }}>
                    <iframe className="absolute inset-0 w-full h-full z-[60]" src={ytEmbed} title={title || 'Trailer'} allow="autoplay; encrypted-media; picture-in-picture" allowFullScreen />
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="flex-1 min-h-0 flex flex-col text-center">
          ...
          <div className="mt-auto pt-2 flex justify-center gap-4">
            <button onClick={() => { setDx(-80); setTimeout(() => end(), 120); }} className="w-28 h-12 rounded-2xl bg-neutral-800 hover:bg-neutral-700"><I.X /> No</button>
            <button onClick={() => { setDx(80); setTimeout(() => end(), 120); }} className="w-28 h-12 rounded-2xl bg-blue-600 hover:bg-blue-500"><I.H /> Yes</button>
          </div>
        </div>
      </div>
    </div>
...
const Results = ({ agreedIds, token, heading = 'Agreed Picks' }: { agreedIds: number[]; token: string; heading?: string; }) => {
  ...
  const [trailers, setT] = useState<Record<number, string>>({});
  const [open, setOpen] = useState<number | null>(null);
  useEffect(() => {
    (async () => {
      const out = await Promise.all(agreedIds.map(async (id) => {
        try {
  ...
  const load = async (id: number) => {
    if (trailers[id]) { setOpen(open === id ? null : id); return; }
    try {
      ...
      const best = chooseBestYT(d.results || []);
      setT({ ...trailers, [id]: best || 'none' });
      setOpen(id);
    } catch { setT({ ...trailers, [id]: 'none' }); setOpen(id); }
  };
            ...
            <li key={m.id} className="p-3 rounded-xl border border-neutral-800 bg-neutral-900/40">
              <div className="flex gap-3">
                {poster ? (
                  ...
                  <div className="w-12 h-18 rounded-md bg-neutral-800 grid place-items-center"><I.M /></div>
                )}
                <div className="flex-1">
                  ...
                  {open === m.id && (
                    <div className="mt-2 text-xs text-neutral-300">
                      <a href={yt} target="_blank" rel="noreferrer noopener" className="underline text-blue-400 hover:text-blue-300">Open trailer on YouTube</a>
                    </div>
                  )}
                </div>
              </div>
...
export default function App() {
  const prefs = {
    region: 'GB', monetization: 'flatrate', providerIds: { netflix: 8, prime: 9 }, minVoteCount: 50, language: 'en-GB', sortBy: 'popularity.desc', targetCount: 10,
  } as const;
  ...
  const [names, setNames] = useState<Record<User, string>>(() => { try { const r = localStorage.getItem('cs_names'); if (r) return JSON.parse(r); } catch {} return { You: '', Partner: '' }; });
  const [s, setS] = useState({
    user: 'You' as User,
    phase: 'welcome' as 'welcome' | 'preDeal' | 'idle' | 'round1' | 'swap' | 'round2' | 'reviewIntro' | 'review1' | 'reviewSwap' | 'review2' | 'noAgreed' | 'final' | 'winner' | 'startOver',
    cohort: [] as MovieLite[],
    idx: { You: 0, Partner: 0 },
  ...
  const [reviewYes, setReviewYes] = useState<{ You: Set<number>; Partner: Set<number> }>({ You: new Set(), Partner: new Set() });
  ...
  useEffect(() => {
    ...
    fetch(`https://api.themoviedb.org/3/genre/movie/list?language=${prefs.language}`, { headers: { Authorization: `Bearer ${token}` } })
      .then((r) => r.json()).then((j) => setGenres(j.genres || [])).catch(() => {});
  }, [token]);
  ...
  async function buildDeck(size?: number) {
    ...
    try {
      ...
      const out: any[] = []; const seen = new Set<number>(); let page = 1;
      while (out.length < target) {
        ...
        const d = await r.json(); const rs: any[] = d.results || [];
        for (const m of rs) {
        ...
        if (out.length >= target) break;
        const tp = Number(d.total_pages || 1); if (page >= tp || rs.length === 0) break; page += 1;
      }
      const cohort: MovieLite[] = out.slice(0, target).map((m: any) => ({ id: m.id, title: m.title, overview: m.overview, year: (m.release_date || '').slice(0, 4), poster: m.poster_path ? IMG + m.poster_path : null, genre_ids: m.genre_ids }));
  ...
  function act(kind: 'like' | 'pass') {
    const cur = s.cohort[s.idx[s.user]]; if (!cur) return; const u = s.user;
    const likes = new Set(s.likes[u]), passes = new Set(s.passes[u]); (kind === 'like' ? likes : passes).add(cur.id);
    let phase = s.phase, agreed = [...s.agreed]; let reviewStarter: User | null = null;
    const nextIdx = s.idx[u] + 1;
    if (s.phase === 'round2' && kind === 'like') { const other: User = u === 'You' ? 'Partner' : 'You'; if (s.likes[other].has(cur.id) && !agreed.includes(cur.id)) agreed.push(cur.id); }
  ...
  const swap = () => setS((v) => { const n: User = v.user === 'You' ? 'Partner' : 'You'; return { ...v, user: n, phase: 'round2', idx: { ...v.idx, [n]: 0 } }; });

  const reviewDone = (u: User, ids: number[]) => {
    const next = new Set(reviewYes[u]); ids.forEach((id) => next.add(id)); const upd = { ...reviewYes, [u]: next } as { You: Set<number>; Partner: Set<number> };
    setReviewYes(upd);
    if (s.phase === 'review1') { setS((v) => ({ ...v, phase: 'reviewSwap' })); return; }
    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-100 p-4">
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          <p className="mb-4">{(s.user === 'You' ? names.You : names.Partner) || 'First person'}'s turn is done — now pass to {(s.user === 'You' ? names.Partner : names.You) || 'Second person'}.</p>
        ...
        <div className="text-center">
          ...
          <div className="mt-6 flex justify-center gap-4">
            ...
            <button onClick={() => act('like')} className="w-28 h-12 rounded-2xl bg-blue-600 hover:bg-blue-500"><I.H /> Yes</button>
          </div>
        </div>
      )}
    </div>
