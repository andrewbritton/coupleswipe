  ...
  return (
    ...
    >
        ...
        <div className="pointer-events-none absolute inset-0 flex items-center justify-between px-3">
          <div
            className={`px-2 py-1 rounded-lg text-xs font-medium ${
              dx < -TH / 2 ? 'bg-red-600/80' : 'bg-transparent'
            }`}
          >
            ← No
          </div>
          <div
            className={`px-2 py-1 rounded-lg text-xs font-medium ${
              dx > TH / 2 ? 'bg-green-600/80' : 'bg-transparent'
            }`}
          >
            Yes →
          </div>
      ...
      <div className="flex-1 min-h-0 flex flex-col">
        <h2 className="text-lg font-semibold mb-1">
          {m.title}{' '}
          {m.year && <span className="text-neutral-400">({m.year})</span>}
        </h2>
        <div className="text-sm text-neutral-300 mb-2 whitespace-pre-wrap break-words flex-1 basis-0 min-h-16 overflow-y-auto pr-1">
    ...
    <div className="max-w-md mx-auto">
      ...
      >
        <div className="w-2/3 mx-auto aspect-[2/3] rounded-lg mb-4 overflow-hidden border border-neutral-800 bg-neutral-800/50 relative">
          ...
          <div className="pointer-events-none absolute bottom-4 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-black/60 border border-white/30 text-white text-xs">
            Play trailer
          </div>
          <div className="pointer-events-none absolute inset-0 flex items-start justify-between p-3">
            <div
              className={`px-2 py-1 rounded-lg text-xs font-medium self-start ${
                dx < -32 ? 'bg-red-600/80' : 'bg-transparent'
              }`}
            >
              ← No
            </div>
            <div
              className={`px-2 py-1 rounded-lg text-xs font-medium self-start ${
                dx > 32 ? 'bg-green-600/80' : 'bg-transparent'
              }`}
            >
              Yes →
            </div>
        ...
        <div className="flex-1 min-h-0 flex flex-col text-center">
          <h3 className="text-base font-medium mb-1">
            {title || 'Untitled'}{' '}
            {rel && <span className="text-neutral-400">{rel}</span>}
          </h3>
          <div className="text-sm text-neutral-300 mb-2 whitespace-pre-wrap break-words flex-1 basis-0 min-h-16 overflow-y-auto pr-1">
          ...
          <div className="mt-auto pt-2 flex justify-center gap-4">
            <button
              ...
              className="w-28 h-12 rounded-2xl bg-neutral-800 hover:bg-neutral-700"
            >
              <I.X /> No
            </button>
            <button
              ...
              className="w-28 h-12 rounded-2xl bg-blue-600 hover:bg-blue-500"
            >
              <I.H /> Yes
            </button>
          </div>
      ...
      <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {items.map((m: any) => {
          ...
          return (
            <li
              key={m.id}
              className="p-3 rounded-xl border border-neutral-800 bg-neutral-900/40"
            >
...
export default function App() {
  ...
  const [token, setToken] = useState(
    () =>
      (import.meta as any)?.env?.VITE_TMDB_TOKEN ||
  ...
  const [reviewYes, setReviewYes] = useState<{ You: Set<number>; Partner: Set<number> }>(
    { You: new Set(), Partner: new Set() },
  );
  ...
  function act(kind: 'like' | 'pass') {
    ...
    const passes = new Set(s.passes[u]);
    (kind === 'like' ? likes : passes).add(cur.id);

    let phase = s.phase;
    let agreed = [...s.agreed];
    let reviewStarter: User | null = null;
    const nextIdx = s.idx[u] + 1;

    if (s.phase === 'round2' && kind === 'like') {
      const other: User = u === 'You' ? 'Partner' : 'You';
    ...
    if (nextIdx >= s.cohort.length) {
      if (s.phase === 'round1') phase = 'swap';
        ...
        if (agreed.length) {
          ...
          const bothNo = idsThis.filter(
            (id) => !s.likes.You.has(id) && !s.likes.Partner.has(id),
          );
          if (bothNo.length) setBan((p) => new Set([...p, ...bothNo]));
          const nx = Math.min(deck + 10, 200);
          setNotice(
            `No agreed picks this round. We'll add ${nx - deck} more ( ${deck} → ${nx} ) and deal again.`,
          );
          setS((v) => ({
            ...v,
            likes: { ...v.likes, [u]: likes },
            passes: { ...v.passes, [u]: passes },
            idx: { ...v.idx, [u]: nextIdx },
            phase: 'noAgreed',
            agreed,
          }));
  ...
  const swap = () =>
    setS((v) => {
      const n: User = v.user === 'You' ? 'Partner' : 'You';
  ...
  const reviewDone = (u: User, ids: number[]) => {
    ...
    const upd = { ...reviewYes, [u]: next } as { You: Set<number>; Partner: Set<number> };
    setReviewYes(upd);
    if (s.phase === 'review1') {
    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-100 p-4">
      <header className="border-b border-neutral-800 pb-2 mb-4 flex items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            onClick={() => buildDeck(deck)}
            className="px-3 py-1.5 bg-neutral-800 hover:bg-neutral-700 rounded-lg"
          >
            <I.R /> Build {deck}
          </button>
        </div>
      ...
      {loading ? (
        ...
        <div className="max-w-xl mx-auto text-center py-10">
          <h2 className="text-xl font-semibold mb-3">
            {(s.user === 'You' ? names.You : names.Partner) || 'Someone'} has been
            randomly chosen to pick first
          </h2>
          <p className="text-sm text-neutral-300">
            When they have made their choices, the other person can make theirs!
          </p>
          <p className="text-sm text-neutral-300 mt-3">
            On the next screen: <strong>swipe right</strong> for{' '}
            <span className="text-green-400 inline-flex items-center gap-1">
              Yes <I.H />
            </span>
            , <strong>swipe left</strong> for{' '}
          ...
          <button
            onClick={() => buildDeck(deck)}
            className="mt-6 px-6 py-3 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white"
          >
            Start picking
          </button>
        </div>
        ...
        <div className="text-center py-20">
          <p className="mb-4">
            {(s.user === 'You' ? names.You : names.Partner) || 'First person'}'s turn is
            done — now pass to{' '}
            {(s.user === 'You' ? names.Partner : names.You) || 'Second person'}.
          </p>
          <button
            onClick={swap}
            className="px-6 py-3 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white"
        ...
        <div className="text-center py-12">
          ...
          <p className="text-sm text-neutral-300 mt-2 mb-6">
            Now you can each watch trailers to make sure you're happy with your pick.
          </p>
          <button
            onClick={() => setS((v) => ({ ...v, phase: 'review1' }))}
            className="px-6 py-3 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white"
          >
            Start {(s.user === 'You' ? names.You : names.Partner) || 'first reviewer'}'s
            trailer review
          </button>
        </div>
      ) : s.phase === 'review1' ? (
        ...
        <div className="text-center py-20">
          <p className="text-lg mb-2">
            {(s.user === 'You' ? names.You : names.Partner) || 'First reviewer'} is done.
          </p>
          <p className="text-sm text-neutral-300 mb-4">
            Now hand the device to{' '}
            {(s.user === 'You' ? names.Partner : names.You) || 'the other person'} so
            they can review the same shortlist.
          </p>
          <button
            onClick={() =>
              setS((v) => ({
                ...v,
                user: v.user === 'You' ? 'Partner' : 'You',
                phase: 'review2',
              }))
            }
            className="px-6 py-3 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white"
          >
            Start {(s.user === 'You' ? names.Partner : names.You) || 'next'}'s trailer
            review
          </button>
        </div>
      ) : s.phase === 'review2' ? (
        ...
        <div className="text-center">
          <Card
            movie={cur}
            genres={genres}
            onSwipe={(d) => act(d === 'right' ? 'like' : 'pass')}
          />
