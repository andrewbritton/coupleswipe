...
// YouTube embed helper (pure)
const ytEmbedUrl = (key?: string | null) => (key ? `https://www.youtube.com/embed/${key}?autoplay=1&rel=0` : 'about:blank');

// --- quick runtime tests (console only) ---
function runTests() {
  const t = (name: string, fn: () => any) => {
    try {
      const r = fn();
  ...
  t('safeMovie defaults', () => {
    const m = safeMovie(null as any);
    if (m.id !== -1 || m.title !== 'Unknown title') throw new Error('defaults');
    return m;
  });
  ...
  // new tests for embed helper
  t('ytEmbedUrl with key', () => ytEmbedUrl('abc123').includes('/embed/abc123'));
  t('ytEmbedUrl no key', () => ytEmbedUrl(null) === 'about:blank');
}
  ...
  return (
    <div className="p-6 rounded-2xl border border-neutral-800 bg-neutral-900/50 max-w-md mx-auto h-[680px] flex flex-col overflow-hidden text-center">
      {poster ? (
        ...
        <div className="w-2/3 mx-auto aspect-[2/3] rounded-lg mb-4 overflow-hidden border border-neutral-800 bg-neutral-800/50 grid place-items-center text-neutral-300 text-sm">
      ...
      <div className="flex-1 min-h-0 flex flex-col justify-between">
        <div>
          <h2 className="text-lg font-semibold mb-1">
            {movie.title}{' '}
            {movie.release_date && <span className="text-neutral-400">({String(movie.release_date).slice(0, 4)})</span>}
          </h2>
        ...
        <div className="mt-auto">
          <a href={tmdbUrl} target="_blank" rel="noreferrer" className="text-xs underline text-blue-400 hover:text-blue-300">
            View on TMDB
          </a>
        </div>
      </div>
...
const Card = ({
  ...
  return (
    <div
      className="p-6 rounded-2xl border border-neutral-800 bg-neutral-900/50 max-w-md mx-auto h-[720px] min-h-0 overflow-hidden flex flex-col select-none"
      onTouchStart={(e) => {
    ...
    >
      <div className="w-2/3 mx-auto aspect-[2/3] rounded-lg mb-4 overflow-hidden border border-neutral-800 bg-neutral-800/50 relative">
        {poster && imgOk ? (
          <img src={poster} alt={m.title} className="w-full h-full object-cover" onError={() => setImgOk(false)} />
        ) : (
      ...
      <div className="flex-1 min-h-0 flex flex-col">
        <h2 className="text-lg font-semibold mb-1">
          {m.title} {m.year && <span className="text-neutral-400">({m.year})</span>}
        </h2>
        ...
        {!!m.genre_ids.length && (
          <div className="flex flex-wrap gap-2 text-xs text-neutral-400 mt-auto pt-2">
            {m.genre_ids.map((id: number) => (
              <span key={id} className="px-2 py-1 rounded-lg bg-neutral-800">
                {g.find((x) => x.id === id)?.name || id}
              </span>
            ))}
          </div>
        )}
...
const TrailerReview = ({ user, token, ids: onIds, onDone }: { user: User; token: string; ids: number[] | undefined; onDone: (ids: number[]) => void }) => {
  ...
  const [i, setI] = useState(0),
    [meta, setMeta] = useState<any | null>(null),
    [tr, setTr] = useState<string | 'none' | null>(null),
    [yes, setYes] = useState<number[]>([]);
  const [dx, setDx] = useState(0);
  const [drag, setDrag] = useState(false);
  const [start, setStart] = useState<number | null>(null);
  ...
  const poster = meta?.poster_path ? IMG + meta.poster_path : null,
    ...
    rel = meta?.release_date ? `(${String(meta.release_date).slice(0, 4)})` : '';
  ...
  const ytEmbed = ytEmbedUrl(tr && tr !== 'none' ? tr : undefined);
  const next = (ok: boolean) => {
    const picks = ok && id ? [...yes, id] : yes;
    if (i + 1 >= ids.length) onDone(picks);
    else {
      setYes(picks);
      setI(i + 1);
    }
  ...
  return (
    <div className="max-w-md mx-auto">
      <h2 className="text-lg font-semibold mb-3 text-center">
        {user}: Trailer review {i + 1} / {ids.length}
      </h2>
      <div
      ...
      >
        <div className="w-2/3 mx-auto aspect-[2/3] rounded-lg mb-4 overflow-hidden border border-neutral-800 bg-neutral-800/50 relative">
          ...
          {tr && tr !== 'none' ? (
            ...
            <a
              ...
              aria-label="Search trailer on YouTube"
              className="absolute z-10 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center justify-center h-24 w-24 rounded-full bg-black/60 border border-white/60 ring-8 ring-white/20 shadow-2xl hover:bg-black/70 focus:outline-none"
            >
              <span className="text-5xl leading-none">
                <I.P />
        ...
        <div className="flex-1 min-h-0 flex flex-col text-center">
          <h3 className="text-base font-medium mb-1">
            {title || 'Untitled'} {rel && <span className="text-neutral-400">{rel}</span>}
          </h3>
          ...
          <div className="mt-auto pt-2 flex justify-center gap-4">
            <button onClick={() => next(false)} className="w-28 h-12 rounded-2xl bg-neutral-800 hover:bg-neutral-700">
              <I.X /> No
            </button>
            <button onClick={() => next(true)} className="w-28 h-12 rounded-2xl bg-blue-600 hover:bg-blue-500">
              <I.H /> Yes
            </button>
          </div>
        </div>
...
const Results = ({ agreedIds, token, heading = 'Agreed Picks' }: { agreedIds: number[]; token: string; heading?: string }) => {
  const [items, setItems] = useState<any[]>([]),
    [trailers, setT] = useState<Record<number, string>>({}),
  ...
  const load = async (id: number) => {
    ...
    try {
      const r = await fetch(`https://api.themoviedb.org/3/movie/${id}/videos?language=en-GB`, { headers: { Authorization: `Bearer ${token}` } });
      const d = r.ok ? await r.json() : { results: [] };
      const yt = (d.results || []).find((v: any) => v.site === 'YouTube' && (v.type === 'Trailer' || v.type === 'Teaser'));
      setT({ ...trailers, [id]: yt ? yt.key : 'none' });
      setOpen(id);
    } catch {
      setT({ ...trailers, [id]: 'none' });
      setOpen(id);
    }
  };
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold mb-4">
        {heading} ({agreedIds.length})
      </h2>
      <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {items.map((m: any) => {
          const poster = m.poster_path ? IMG + m.poster_path : null,
            tmdb = `https://www.themoviedb.org/movie/${m.id}`;
          const k = trailers[m.id];
          const yt = k && k !== 'none' ? `https://www.youtube.com/watch?v=${k}` : `https://www.youtube.com/results?search_query=${encodeURIComponent((m.title || '') + ' trailer')}`;
          return (
            <li key={m.id} className="p-3 rounded-xl border border-neutral-800 bg-neutral-900/40">
              <div className="flex gap-3">
                {poster ? (
                  <img src={poster} alt={m.title} className="w-12 h-18 object-cover rounded-md border border-neutral-800" />
                ) : (
                  <div className="w-12 h-18 rounded-md bg-neutral-800 grid place-items-center">
                    <I.M />
                  </div>
                )}
                <div className="flex-1">
                  ...
                  <div className="mt-1 flex gap-2">
                    <button onClick={() => load(m.id)} className="text-xs text-blue-400 underline hover:text-blue-300">
                      {open === m.id ? 'Hide trailer' : 'Play trailer'}
                    </button>
                    <a href={tmdb} target="_blank" rel="noreferrer" className="text-xs underline text-blue-400 hover:text-blue-300">
                      View on TMDB
                    </a>
                  </div>
                  {open === m.id && (
                    <div className="mt-2 text-xs text-neutral-300">
                      <a href={yt} target="_blank" rel="noreferrer noopener" className="underline text-blue-400 hover:text-blue-300">
  ...
  return (
    <div className="max-w-md mx-auto text-center">
      ...
      <div className="mt-3 flex justify-center gap-3">
        <button onClick={onBack} className="px-4 py-2 rounded-xl bg-neutral-800 hover:bg-neutral-700">
          Back to list
        </button>
...
export default function App() {
  ...
  const [token, setToken] = useState(() => (import.meta as any)?.env?.VITE_TMDB_TOKEN || localStorage.getItem('tmdb_v4_token') || ''),
    ...
    [deck, setDeck] = useState<number>(prefs.targetCount),
    [ban, setBan] = useState<Set<number>>(new Set()),
    [genres, setGenres] = useState<{ id: number; name: string }[]>([]),
    [names, setNames] = useState<Record<User, string>>(() => {
      try {
        const r = localStorage.getItem('cs_names');
  ...
  async function buildDeck(size?: number) {
    ...
    setNotice('');
    try {
      const target = (typeof size === 'number' ? size : deck) || 10;
      const out: any[] = [];
      const seen = new Set<number>();
      let page = 1;
      while (out.length < target) {
        const url = `https://api.themoviedb.org/3/discover/movie?watch_region=${prefs.region}&include_adult=false&sort_by=${prefs.sortBy}&page=${page}&with_watch_providers=${prefs.providerIds.netflix}|${prefs.providerIds.prime}&with_watch_monetization_types=${prefs.monetization}&vote_count.gte=${prefs.minVoteCount}`;
        const r = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
        if (!r.ok) throw new Error('TMDB fetch failed: ' + r.status);
        const d = (await r.json()) as any;
        const rs: any[] = Array.isArray(d.results) ? d.results : [];
        for (const m of rs) {
          if (ban.has(m.id)) continue;
          if (!seen.has(m.id)) {
            seen.add(m.id);
            out.push(m);
          }
        ...
        if (out.length >= target) break;
        const tp = Number(d.total_pages || 1);
  ...
  function act(kind: 'like' | 'pass') {
    ...
    if (!cur) return;
    const u = s.user;
    const likes = new Set(s.likes[u]),
      passes = new Set(s.passes[u]);
    (kind === 'like' ? likes : passes).add(cur.id);
    let phase = s.phase,
      agreed = [...s.agreed];
    let reviewStarter: null | User = null;
    const nextIdx = s.idx[u] + 1;
    if (s.phase === 'round2' && kind === 'like') {
      const other = u === 'You' ? 'Partner' : 'You';
      if (s.likes[other].has(cur.id) && !agreed.includes(cur.id)) agreed.push(cur.id);
    }
    if (nextIdx >= s.cohort.length) {
      if (s.phase === 'round1') phase = 'swap';
      else if (s.phase === 'round2') {
        if (agreed.length) {
          reviewStarter = Math.random() < 0.5 ? 'You' : 'Partner';
          phase = 'reviewIntro';
        } else {
          const idsThis = s.cohort.map((m) => m.id);
          const bothNo = idsThis.filter((id) => !s.likes.You.has(id) && !s.likes.Partner.has(id));
          if (bothNo.length) setBan((p) => new Set([...p, ...bothNo]));
          const nx = Math.min(deck + 10, 200);
          setNotice(`No agreed picks this round. We'll add ${nx - deck} more ( ${deck} → ${nx} ) and deal again.`);
          setS((v) => ({ ...v, likes: { ...v.likes, [u]: likes }, passes: { ...v.passes, [u]: passes }, idx: { ...v.idx, [u]: nextIdx }, phase: 'noAgreed', agreed }));
          return;
        }
      }
    }
  ...
  const swap = () =>
    setS((v) => {
      const n = v.user === 'You' ? 'Partner' : 'You';
      return { ...v, user: n, phase: 'round2', idx: { ...v.idx, [n]: 0 } };
    });
  const reviewDone = (u: User, ids: number[]) => {
    const next = new Set(reviewYes[u]);
    ids.forEach((id) => next.add(id));
    const upd = { ...reviewYes, [u]: next } as any;
    setReviewYes(upd);
    if (s.phase === 'review1') {
      setS((v) => ({ ...v, phase: 'reviewSwap' }));
      return;
    }
    const finals = s.agreed.filter((id) => upd.You.has(id) && upd.Partner.has(id));
    setS((v) => ({ ...v, phase: finals.length ? 'final' : 'startOver', agreed: finals }));
  };

  const cur = s.cohort[s.idx[s.user]];
  const pick = () => {
    if (!s.agreed.length) return;
    const id = s.agreed[Math.floor(Math.random() * s.agreed.length)];
    setWin(id);
    setS((v) => ({ ...v, phase: 'winner' }));
  };
  const expand = () => {
    const nx = Math.min(deck + 10, 200);
    setDeck(nx);
  ...
  const fresh = () => {
    const seen = new Set<number>();
    s.cohort.forEach((m) => seen.add(m.id));
    s.likes.You.forEach(seen.add, seen);
    s.likes.Partner.forEach(seen.add, seen);
    s.passes.You.forEach(seen.add, seen);
    s.passes.Partner.forEach(seen.add, seen);
    s.agreed.forEach(seen.add, seen);
    setBan((p) => new Set([...p, ...seen]));
    setS({ user: 'You', phase: 'idle', cohort: [], idx: { You: 0, Partner: 0 }, likes: { You: new Set(), Partner: new Set() }, passes: { You: new Set(), Partner: new Set() }, agreed: [] });
  ...
  const restart = () => {
    ...
    setBan(new Set());
    setS({ user: 'You', phase: 'welcome', cohort: [], idx: { You: 0, Partner: 0 }, likes: { You: new Set(), Partner: new Set() }, passes: { You: new Set(), Partner: new Set() }, agreed: [] });
  };
    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-100 p-4">
      <header className="border-b border-neutral-800 pb-2 mb-4 flex items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button onClick={() => buildDeck(deck)} className="px-3 py-1.5 bg-neutral-800 hover:bg-neutral-700 rounded-lg">
            <I.R /> Build {deck}
          </button>
        </div>
      </header>
      ...
      <div className="mb-4 flex items-center gap-2">
        ...
        <input
          ...
          onChange={(e) => {
            ...
            setToken(v);
            try {
              localStorage.setItem('tmdb_v4_token', v);
            } catch {}
          }}
      ...
      {loading ? (
        ...
        <div className="max-w-md mx-auto text-center py-10">
          ...
          <div className="space-y-3 text-left">
            ...
            <input
              value={names.You}
              onChange={(e) => {
                const next = { ...names, You: e.target.value };
                setNames(next);
                try {
                  localStorage.setItem('cs_names', JSON.stringify(next));
                } catch {}
              }}
            ...
            <input
              value={names.Partner}
              onChange={(e) => {
                const next = { ...names, Partner: e.target.value };
                setNames(next);
                try {
                  localStorage.setItem('cs_names', JSON.stringify(next));
                } catch {}
          ...
          <div className="mt-4 flex items-center justify-center gap-3">
            <button
              onClick={() => {
                if (!names.You.trim() || !names.Partner.trim()) return;
                setS((v) => ({ ...v, user: Math.random() < 0.5 ? 'You' : 'Partner', phase: 'preDeal' }));
              }}
            ...
            {(names.You || names.Partner) && (
              <button
                onClick={() => {
                  ...
                  try {
                    localStorage.removeItem('cs_names');
                  } catch {}
                }}
        ...
        <div className="max-w-xl mx-auto text-center py-10">
          ...
          <p className="text-sm text-neutral-300 mt-3">
            On the next screen: <strong>swipe right</strong> for <span className="text-green-400 inline-flex items-center gap-1">Yes <I.H /></span>, <strong>swipe left</strong> for <span className="text-red-400 inline-flex items-center gap-1">No <I.X /></span>. You can also use the buttons.
          </p>
          <button onClick={() => buildDeck(deck)} className="mt-6 px-6 py-3 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white">
            Start picking
          </button>
        </div>
      ) : s.phase === 'swap' ? (
        <div className="text-center py-20">
          <p className="text-lg mb-4">
            <I.F /> {(s.user === 'You' ? names.You : names.Partner) || 'First person'}'s turn is done — now pass to {(s.user === 'You' ? names.Partner : names.You) || 'Second person'}.
          </p>
          <button onClick={swap} className="px-6 py-3 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white">
            Start {(s.user === 'You' ? names.Partner : names.You) || 'partner'}'s turn
          </button>
        </div>
      ) : s.phase === 'reviewIntro' ? (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-neutral-300">
            You both like the look of <strong>{s.agreed.length}</strong> movie{s.agreed.length === 1 ? '' : 's'}.
          </p>
          ...
          <button onClick={() => setS((v) => ({ ...v, phase: 'review1' }))} className="px-6 py-3 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white">
            Start {(s.user === 'You' ? names.You : names.Partner) || 'first reviewer'}'s trailer review
          </button>
        </div>
      ) : s.phase === 'review1' ? (
        ...
        <div className="text-center py-20">
          ...
          <button onClick={() => setS((v) => ({ ...v, user: v.user === 'You' ? 'Partner' : 'You', phase: 'review2' }))} className="px-6 py-3 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white">
            Start {(s.user === 'You' ? names.Partner : names.You) || 'next'}'s trailer review
          </button>
        </div>
      ) : s.phase === 'review2' ? (
        <TrailerReview user={s.user} token={token} ids={s.agreed} onDone={(ids) => reviewDone(s.user, ids)} />
        ...
        <div>
          ...
          <div className="mt-6 p-4 rounded-2xl border border-neutral-800 bg-neutral-900/50 text-center">
            ...
            <button onClick={pick} disabled={!s.agreed.length} className="px-6 py-3 rounded-2xl bg-blue-600 hover:bg-blue-500 disabled:opacity-50 disabled:cursor-not-allowed">
              Random pick
            </button>
          </div>
        </div>
